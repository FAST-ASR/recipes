...
begin
	md"""
	# Lattice-Free MMI II: Training the Model
	*[Lucas Ondel](https://lucasondel.github.io/index), 28th October 2021*
...
Importantly, we use the following package:
...
md"""
We load the configuration file of the experiment. By default, we look for the file  `config.toml` in the root directory (i.e. the directory containing this notebook). Alternatively, when calling this notebook as a julia script, you can specify another configuration file by calling: 
```
$ SPEECHLAB_LFMMI_CONFIG=/path/to/file julia 'LFMMI II: Training.jl'
```
"""
...
|   |   +-- <features.name>.h5
```
where `<features.dir>`, `<features.name>`, `<dataset.{train,dev,test}` and `<dataset.name>` are taken from the configuration file.
!!! note
...
featrain = joinpath(feadir, config["dataset"]["train"], feaname * ".h5")

# ╔═╡ 208c0a95-43ca-420c-b349-ac12da9631e4
feadev = joinpath(feadir, config["dataset"]["dev"], feaname * ".h5")

# ╔═╡ 907ad799-2f95-4b68-9033-7cc1d8853f00
featest = joinpath(feadir, config["dataset"]["test"], feaname * ".h5")
...
!!! note
	`best.jld2` is the model with the best loss function evaluated on the development set whereas `checkpoint.jld2` is the model at the last epoch..

"""

# ╔═╡ eaa248f5-b8f3-4e3a-93a3-d4a275929878
expdir = joinpath(rootdir, config["experiment"]["dir"], config["dataset"]["name"])
