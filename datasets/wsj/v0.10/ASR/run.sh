...
dir=exp/${arch,,}_${type}${unit}${affix:+_$affix}
...
if [ ${stage} -le 4 ]; then
  ...
  train_fst=""; [ $loss == "chain" ] && train_fst=$graph/$train_set/num.scp
  ...
  valid_fst=""; [ $loss == "chain" ] && valid_fst=$graph/$valid_set/num.scp
...
if [ ${stage} -le 5 ]; then
  ...
  if [ $loss != "chain" ]; then
    ...
    train_script="train.py --den-fst $graph/normalization.fst"
  fi

#  $cuda_cmd log/train.log \
#    PATH="/mnt/matylda3/ikocour/tools/miniconda3/envs/scmrnn/bin:$PATH" \
