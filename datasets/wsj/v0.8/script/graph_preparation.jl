...
begin
	import Pkg
	Pkg.activate("/mnt/matylda3/ikocour/SCMR/WSJ_mix/notebooks")
	using JLD2
	using MarkovModels
	using PlutoUI
	using Random
...
begin
	md"""
	# Lattice-Free MMI I: Preparing the Graphs
	*[Lucas Ondel](https://lucasondel.github.io/), 28th October 2021*
...
md"""
Notably, we use the [MarkovModels](https://github.com/lucasondel/MarkovModels.jl) package to build our graphs.
"""
...
```
SPEECHLAB_LFMMI_CONFIG=/path/to/file julia 'LFMMI I: Preparing the Graphs.jl'
```

An example of configuration can be found [here](https://lucasondel.github.io/resources/SpeechLab/lfmmi/v1/config).
"""

# ╔═╡ d8799061-1dab-402f-8997-0b89ae43c07e
pathof(MarkovModels)
...
configfile = get(ENV, "SPEECHLAB_LFMMI_CONFIG", joinpath(rootdir, "./conf/config.toml"))
...
### Transcription
The text transcription is simply a text file with one line per utterance:
```
utt1  w1 w2 w3 ...
utt2  w1 w2 w3 ...
...
...
#					   "lang", "lexicon")
lexiconfile = joinpath(rootdir, config["graphs"]["lexicon"])

# ╔═╡ 537679cc-3104-4bbb-aab8-21d80ed03ced
#unitsfile = joinpath(config["dataset"]["dir"], config["dataset"]["name"],
#					 "lang", "units")
unitsfile = joinpath(rootdir, config["graphs"]["units"])
...
# ╔═╡ a155296a-388a-4a82-b5dc-f24dfbc931bf
train = config["dataset"]["train"]

# ╔═╡ de3aa3b0-f586-4ae3-8b59-ad64de7153f1
dev = config["dataset"]["dev"]

# ╔═╡ cf09396d-5c3e-4143-b17f-32fda149d45a
test = config["dataset"]["test"]

# ╔═╡ cf5fc8b6-e14d-49b2-9c7d-c8e58e80c118
md"""
We have two options to generate the graphs: either we build the graph by "sampling" an alignment sequence (this is the Kaldi way) or we build the graph of all possible sequences (recommended way). The generating mode is decided by the configuration key `<graphs.sample>`.
"""
...
The parameters are:
  ...
  * `between_silprob` the probability to have silence between words
...
# ╔═╡ bdea72b2-95e7-4f36-ac3d-d887ae73d824
lexicon
...
function makenums(fout, trans, lexicon, sil_sym, oov_sym, between_silprob,
				  edge_silprob, ngram_order; sample=false)

	ngram_counts = Dict()
	open(trans, "r") do f
        for line in eachline(f)
			uttid, tokens... = split(line)
...
ngram_counts = jldopen(train_alifile, "w") do fout
	@info "Generating alignments FSMs for the train set (this may take a while)."
	makenums(fout, train_trans, lexicon, sil_sym, oov_sym, between_silprob,
			 edge_silprob, ngram_order; sample)
end
...
jldopen(dev_alifile, "w") do fout
	@info "Generating alignments FSMs for the dev set (this may take a while)."
	makenums(fout, dev_trans, lexicon, sil_sym, oov_sym, between_silprob,
			 edge_silprob, ngram_order; sample=sample)
end
