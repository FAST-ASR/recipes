...
decode_dir=exp/tdnn_monophone${nnet_affix}/decode_${eval_set}_${lang_test#*lang_test_}_b${beam}_lb${lbeam}_actw${acwt}
posteriors=exp/tdnn_monophone${nnet_affix}/${eval_set}/posteriors.ark

if [ $stage -le -2 ]; then
    mkdir -p "$lang/local/dict"
    sort -u ../data/lang/lexicon.txt >$lang/local/dict/lexicon.txt
    cat >$lang/local/dict/silence_phones.txt <<EOF
SIL
NSN
SPN
EOF
    egrep -v 'SIL|NSN|SPN' ../data/lang/units.txt >$lang/local/dict/nonsilence_phones.txt
...
if [ $stage -le 1 ]; then
    ...
    (
        ...
        cd $WSJ_ROOT  
        local/score.sh data/$eval_set $graph $decode_dir
    )
...
if [ $stage -le 2 ]; then
    ...
    (
        ...
        decode_dir=$(realpath $decode_dir)
        cd $WSJ_ROOT  
        local/score.sh data/$eval_set $graph ${decode_dir}_lmrescore-fgconst
    )
